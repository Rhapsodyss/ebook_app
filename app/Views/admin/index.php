...
<body class="bg-ink text-cream min-h-screen">
    ...
    <!-- NAVBAR -->
    <nav class="border-b border-white/10 bg-ink/90 backdrop-blur-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-3 py-4 flex items-center justify-between">
            ...
            <a href="/" class="text-sm text-white/40 hover:text-amber transition-colors">
                ← Beranda
            </a>
    ...
    <div class="max-w-6xl mx-auto px-6 py-10">
        ...
        <div class="mb-8">
            ...
            <h1 class="font-display text-3xl font-black">
                <?= isset($edit) ? 'Edit Buku' : 'Kelola Buku' ?>
            </h1>
        </div>
        ...
        ═══════════════════════════ -->
            <div class="bg-inklight rounded-2xl p-6 border border-white/10">
                ...
                <h2 class="font-display text-lg font-bold mb-6 pb-4 border-b border-white/10">
                    <?= isset($edit) ? '✏️ Edit Buku' : '＋ Tambah Buku' ?>
                </h2>
                ...
                <form
                    ...
                    <div class="field">
                        ...
                        <textarea name="description" rows="2"
                            placeholder="Deskripsi singkat..."><?= htmlspecialchars($edit['description'] ?? '') ?></textarea>
                    </div>

                    <div class="field">
                        <label class="block text-xs text-white/40 uppercase tracking-wider mb-1.5">Detail Buku</label>
                        <textarea name="story_detail" rows="8"
                            placeholder="Isi Buku lengkap..."><?= htmlspecialchars($edit['story_detail'] ?? '') ?></textarea>
                    </div>

                    <div class="flex items-center gap-3 pt-2">
                        <button type="submit"
                            class="bg-amber text-ink font-medium text-sm px-6 py-2.5 rounded-full hover:bg-amber/90 transition-colors">
                            <?= isset($edit) ? 'Simpan Perubahan' : 'Tambahkan Buku' ?>
                        </button>
